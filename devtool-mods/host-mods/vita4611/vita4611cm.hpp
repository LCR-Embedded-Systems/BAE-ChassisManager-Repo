...
using warnThresholdIface = sdbusplus::xyz::openbmc_project::Sensor::Threshold::server::Warning;
...
struct chassis_IPMB_description_record
{
    uint8_t Record_Type_ID;
    uint8_t End_of_List;
    uint8_t Record_Length;
    uint8_t Record_Checksum;
    uint8_t Header_Checksum;
    uint8_t Manufacturer_ID_1;
    uint8_t Manufacturer_ID_2;
    uint8_t Manufacturer_ID_3;
    uint8_t Vita_Record_ID;
    uint8_t Record_Format_Version;
    uint8_t IPMB_Support;
};
...
struct SensorRecord
{
    ...
    std::unique_ptr<sdbusplus::server::object_t<ValueIface, AssociationIface, ThresholdIface, warnThresholdIface>> sensorIface;
...
public:
    ...
    ipmi::RspType<std::vector<uint8_t>> getShMCVSOCaps(); //done
    ipmi::RspType<std::vector<uint8_t>> getAddTable(const std::vector<uint8_t>& reqdata); //done
    ipmi::RspType<std::vector<uint8_t>> getChassisIdentifier(); //done
    ipmi::RspType<std::vector<uint8_t>> setChassisIdentifier(const std::vector<uint8_t>& reqdata); //done
    ipmi::RspType<std::vector<uint8_t>> fruControl(const std::vector<uint8_t>& reqdata); //done
    ipmi::RspType<std::vector<uint8_t>> setIpmbState(const std::vector<uint8_t>& reqdata); //done
    ipmi::RspType<std::vector<uint8_t>> setFruStatePolicyBits(const std::vector<uint8_t>& reqdata); //done
    ipmi::RspType<std::vector<uint8_t>> getFruStatePolicyBits(uint8_t devId); //done
    ipmi::RspType<std::vector<uint8_t>> setFruActivation(const std::vector<uint8_t>& reqdata); //done
    ipmi::RspType<std::vector<uint8_t>> getDeviceLocatorRecordId(const std::vector<uint8_t>& reqdata);
    ipmi::RspType<std::vector<uint8_t>> getChassisManagerIpmbAddress(); //done

    ipmi::RspType<std::vector<uint8_t>> setFanPolicy(const std::vector<uint8_t>& reqdata); // partially done
    ipmi::RspType<std::vector<uint8_t>> getFanPolicy(const std::vector<uint8_t>& reqdata); // done

    ipmi::RspType<std::vector<uint8_t>> fruControlCapabilities();
    ipmi::RspType<std::vector<uint8_t>> fruInventoryDeviceLockControl();
    ipmi::RspType<std::vector<uint8_t>> fruInventoryDeviceWrite();

    ipmi::RspType<std::vector<uint8_t>> getChassisManagerIpAddresses(uint8_t addr_num); //done

    //Helper function
    ipmi::RspType<std::vector<uint8_t>> get_chm_fru_info();

    ipmi::RspType<std::vector<uint8_t>> getFruAddressInfo(const std::vector<uint8_t>& reqdata);
    ipmi::RspType<std::vector<uint8_t>> getMandatorySensorNumbers();

    //Mandatory LAN Device Commands
    ipmi::RspType<std::vector<uint8_t>> setLanConfigurationParams();
    ipmi::RspType<std::vector<uint8_t>> getLanConfigurationParams();

    //Mandatory Fan Commands
    ipmi::RspType<std::vector<uint8_t>> getFanSpeedProperties(const std::vector<uint8_t>& reqdata); // done
    ipmi::RspType<std::vector<uint8_t>> getFanLevel(const std::vector<uint8_t>& reqdata); // done; TODO: make byte 5 reflect status of fan controller service
    ipmi::RspType<std::vector<uint8_t>> setFanLevel(const std::vector<uint8_t>& reqdata);
    ...
    ipmi::RspType<std::vector<uint8_t>> handleChMCommand(const std::vector<uint8_t>& reqdata);
...
private:
    ...
    */
    ...
    sdbusplus::server::manager_t sensorsManager;

    std::map<std::string, std::unique_ptr<sdbusplus::server::object_t<ValueIface, AssociationIface, ThresholdIface, warnThresholdIface>>> sensorIfaces;

    bool ipmb_b_enabled;

    chassis_IPMB_description_record *IPMB_inst;

    bool IPMB_chassis_supported;
    bool IPMB_A_400khz;
    bool IPMB_B_400khz;
