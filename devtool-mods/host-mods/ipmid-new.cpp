...
message::Response::ptr executeIpmiCommand(message::Request::ptr request)
{
    ...
    // std::string msg = std::string("in execute ipmi command now with netFn - ") + std::to_string((int)netFn);
...
{
    // log<level::INFO>("executionEntry enter");
    ...
    // log<level::INFO>(("SecureBuffer contents data = " + oss.str()).c_str());
