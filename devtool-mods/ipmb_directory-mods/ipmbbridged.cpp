...
{
    // phosphor::logging::log<phosphor::logging::level::INFO>("processI2cEvent: currently processing an i2c event.");
    ...
    {
        // phosphor::logging::log<phosphor::logging::level::INFO>("processI2cEvent: message received from the ipmb channel.");
...
int IpmbChannel::ipmbChannelInit(const char* ipmbI2cSlave)
{
    ...
    phosphor::logging::log<phosphor::logging::level::INFO>("ipmbChannelInit: opening the slave device now." );
                            ...
                            std::shared_ptr<IpmbRequest> request)
{
    ...
    // std::string logMsg1 = "requestAdd: in request Add.";
    ...
    // std::stringstream ss;
    // ss << "requestAdd: Buffer contents (hex): ";
    ...
    for (int i = 0; i < ipmbNumberOfTries; i++)
    {
        ...
        // std::string logMsg2 = "requestAdd: number of retries: " + std::to_string(i);
        // phosphor::logging::log<phosphor::logging::level::INFO>(logMsg2.c_str());
...
static int initializeChannels()
{
    ...
    try
    {
        ...
        for (const auto& channelConfig : data["channels"])
        {
            ...
            std::string logMsg = "initializeChannels: creating/updating channel - "
                        "type=" + typeConfig +
                        ", slavePath=" + slavePath +
                        ", bmcAddr=" + std::to_string(bmcAddr) +
                        ", remoteAddr=" + std::to_string(reqAddr) +
                        ", devIndex=" + std::to_string(devIndex);
            phosphor::logging::log<phosphor::logging::level::INFO>(logMsg.c_str());
...
void IpmbChannel::updateRqSlaveAddress(uint8_t addr) {

    ipmbRqSlaveAddress = addr;

}

auto ipmbHandleRequest = [](boost::asio::yield_context yield,
                            uint8_t reqChannel, uint8_t addr, uint8_t netfn, uint8_t lun,
    ...
    */
    ...
    // std::string logMsg2 = "ipmbHandleRequest: channel slave address is: " + std::to_string(rqSlaveAddress)
    // + " and BMC address is: " + std::to_string(bmcSlaveAddress);
    ...
    // std::string logMsg3 = "ipmbHandleRequest: created a new request, adding that to request list now.";
