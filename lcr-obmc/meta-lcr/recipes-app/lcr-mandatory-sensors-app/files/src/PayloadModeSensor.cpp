...
/*
####################################################
PAYLOAD MODE SENSOR DEFINITION
####################################################
*/

PayloadModeSensor::PayloadModeSensor(std::shared_ptr<sdbusplus::asio::connection> conn_ref,
    ...
    phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor initialization");
    sensor_path = "/xyz/openbmc_project/sensors/mandatoryNumbers/FRU_MODE";
}

PayloadModeSensor::~PayloadModeSensor() {
    timer.cancel();
}

void PayloadModeSensor::emit_value_changed() {
    phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor emit_value_changed enter");
    ...
    conn->async_send(msg, [](const boost::system::error_code& ec, sdbusplus::message::message& m) {
        if (ec) {
            phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor emit_value_changed async send failure");
        }
    });
    phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor emit_value_changed exit");
}

void PayloadModeSensor::expose_sensor() {
    phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor expose_sensor enter");
    ...
    phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor expose_sensor exit");
}

void PayloadModeSensor::async_update_interface() {
    // phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor async_update_interface enter");
    async_get_payload_mode([this](uint8_t val) {
    ...
    // phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor async_update_interface exit");
}
// Schedule the next periodic update
void PayloadModeSensor::schedule_update() {
    // phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor schedule_update enter");
    timer.expires_after(std::chrono::milliseconds(1000));
    timer.async_wait([this](const boost::system::error_code& ec) {
        if (ec) {
            phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor schedule_update async wait failure. failed to schedule update");
            return;
        }
        this->async_update_interface();
    });
    // phosphor::logging::log<phosphor::logging::level::INFO>("PayloadModeSensor schedule_update exit");
}

// Asynchronous get_all_alarms_status with short-circuit on first alarm
void PayloadModeSensor::async_get_payload_mode(std::function<void(uint8_t)> callback) {
    boost::asio::post(conn->get_io_context(), [this, callback]() {
        uint8_t result = 0x02;  // default = no flag / normal

        try {
            std::ifstream file("/usr/payload_test/payload_mode_flag");
            if (file.is_open()) {
                int flag_value = 0;
                if (file >> flag_value) {
                    if (flag_value == 1) {
            ...
            phosphor::logging::log<phosphor::logging::level::ERR>(
                "Failed to read payload_mode_flag file",
...
void PayloadModeSensor::start_updates() {
    schedule_update();
}