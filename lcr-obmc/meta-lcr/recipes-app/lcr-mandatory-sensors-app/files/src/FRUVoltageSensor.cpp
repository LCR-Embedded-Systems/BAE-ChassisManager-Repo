...
FRUVoltageSensor::FRUVoltageSensor(std::shared_ptr<sdbusplus::asio::connection> conn_ref,
    ...
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor initialization");
    sensor_path = "/xyz/openbmc_project/sensors/mandatoryNumbers/VSO_VOLTAGE";
    byte3 = static_cast<double>(1 << 5);
}

FRUVoltageSensor::~FRUVoltageSensor() {
    timer.cancel();
}

void FRUVoltageSensor::emit_value_changed() {
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor emit_value_changed enter");
    ...
    conn->async_send(msg, [](const boost::system::error_code& ec, sdbusplus::message::message& m) {
        if (ec) {
            phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor emit_value_changed async send failure");
        }
    });
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor emit_value_changed exit");
}

void FRUVoltageSensor::expose_sensor() {
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor expose_sensor enter");
    ...
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor expose_sensor exit");
}

void FRUVoltageSensor::async_update_interface() {
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor async_update_interface enter");
    ...
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor async_update_interface exit");
}
// Schedule the next periodic update
void FRUVoltageSensor::schedule_update() {
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor schedule_update enter");
    timer.expires_after(std::chrono::milliseconds(1000));
    timer.async_wait([this](const boost::system::error_code& ec) {
        if (ec) {
            phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor schedule_update async wait failure. failed to schedule update");
            return;
        }
        this->async_update_interface();
    });
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor schedule_update exit");
}

// Asynchronous get_all_alarms_status with short-circuit on first alarm
void FRUVoltageSensor::async_get_all_alarms_status(std::function<void(uint8_t)> callback) {
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUVoltageSensor async_get_all_alarms_status enter");
...
void FRUVoltageSensor::start_updates() {
    schedule_update();
}