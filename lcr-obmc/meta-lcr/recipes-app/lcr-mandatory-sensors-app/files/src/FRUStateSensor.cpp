...
/*
####################################################
FRU STATE DEFINITION
####################################################
*/

FRUStateSensor::FRUStateSensor(std::shared_ptr<sdbusplus::asio::connection> conn_ref,
    ...
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor initialization");
    sensor_path = "/xyz/openbmc_project/sensors/mandatoryNumbers/VSO_FRU_STATE";
}

FRUStateSensor::~FRUStateSensor() {
    timer.cancel();
}

void FRUStateSensor::emit_value_changed() {
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor emit_value_changed enter");
    ...
    conn->async_send(msg, [](const boost::system::error_code& ec, sdbusplus::message::message& m) {
        if (ec) {
            phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor emit_value_changed async send failure");
        }
    });
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor emit_value_changed exit");
}

void FRUStateSensor::expose_sensor() {
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor expose_sensor enter");
    ...
    phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor expose_sensor exit");
}

void FRUStateSensor::async_update_interface() {
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor async_update_interface enter");
    async_get_all_alarms_status([this](uint8_t val) {
        double new_val = static_cast<double>(val);
        if (new_val != this->current_value) {
    ...
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor async_update_interface exit");
}
// Schedule the next periodic update
void FRUStateSensor::schedule_update() {
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor schedule_update enter");
    timer.expires_after(std::chrono::milliseconds(1000));
    timer.async_wait([this](const boost::system::error_code& ec) {
        if (ec) {
            phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor schedule_update async wait failure. failed to schedule update");
            return;
        }
        this->async_update_interface();
    });
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor schedule_update exit");
}

// Asynchronous get_all_alarms_status with short-circuit on first alarm
void FRUStateSensor::async_get_all_alarms_status(std::function<void(uint8_t)> callback) {
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor async_get_all_alarms_status enter");
    ...
    *check_ptr = [this, stubs, check_next, alarm_active, done, threshold_interface, check_ptr]() mutable {
        if (*alarm_active || *check_next >= stubs.size()) {
            // phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor async_get_all_alarms_status finished checking alarms. running done");
            done();
            return;
        }
        ...
        phosphor::interface::util::async_alarm_check(conn, stub, threshold_interface, 0,
            [alarm_active, check_ptr](bool res) {
                if (res) {
                    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor async_get_all_alarms_status an alarm is set");
                    *alarm_active = true;
                }
                (*check_ptr)();
            });
    ...
    (*check_ptr)();
    // phosphor::logging::log<phosphor::logging::level::INFO>("FRUStateSensor async_get_all_alarms_status exit");
}

// Start the asynchronous update cycle
void FRUStateSensor::start_updates() {
    schedule_update();
}