...
class PayloadTestStatus {
    ...
    public: 
        PayloadTestStatus(std::shared_ptr<sdbusplus::asio::connection> conn_ref, sdbusplus::asio::object_server& server_ref);
        ~PayloadTestStatus();
        void async_get_payload_status(std::function<void(uint8_t)> callback);
        void start_updates();
        void expose_sensor();
