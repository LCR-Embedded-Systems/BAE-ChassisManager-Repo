...
class PayloadTestResults {
    ...
    public: 
        PayloadTestResults(std::shared_ptr<sdbusplus::asio::connection> conn_ref, sdbusplus::asio::object_server& server_ref);
        ~PayloadTestResults();
        void async_get_payload_results(std::function<void(uint8_t)> callback);
        void start_updates();
        void expose_sensor();
