...
constexpr auto phosphorServiceName = "LCR_fan_controller.service";
...
std::string getNumber(std::string originalString) {
                      ...
                      bool shortPath = false)
{
         ...
         SDBusPlus::getSubTreePathsRaw(SDBusPlus::getBus(), path, iface, 0))
    {
        for (auto& fan : fans)
        {
            if (shortPath)
            {
                if (getNumber(fan) == getNumber(justFanName(path)))
                {
                    dest[fan].push_back(path);
                }
            }
            else if (getNumber(fan) == getNumber(justFanName(path)))
            {
                dest[fan].push_back(path);
            }
        }
...
auto loadDBusData()
{
    ...
    std::map<const std::string, const std::string> paths{
        ...
        {"control", "/xyz/openbmc_project/control"}};

    // build a list of all f
    ...
    if (0 == fanNames.size())
    {
        ...
        for (auto& path : SDBusPlus::getSubTreePathsRaw(
                 bus, paths["control"], interfaces["FanPwm"], 0))
        {
            // special case where we build the list of fans
            auto fan = justFanName(path);
            fan = fan.substr(0, fan.rfind("_"));
    ...
    pathMap["opstatus"] = getPathsFromIface(
        paths["tach"], interfaces["OpStatus"], fanNames, true);

    //Load pwm setting paths for each fan    
    pathMap["pwm"] =
        getPathsFromIface(paths["control"], interfaces["FanPwm"], fanNames);
        
    std::cout << "loadDBusData exit" << std::endl;
...
{
    log<level::INFO>("Fanctl getStates enter");
    using DBusTuple =
    ...
    try
    {
        ...
        {
            std::cout << "No units found for systemd service: " << services[0]<< std::endl;
        }
    }
...
void status()
{
    std::cout << "status enter" << std::endl;
    using std::cout;
    using std::endl;
    using std::setw;
    ...
    std::cout << "status getting maps" << std::endl;
...
void get()
{
    ...
    for (auto& fan : fanNames)
    {
        ...
        property = "Target";
        cout << SDBusPlus::getProperty<uint64_t>(
            pathMap["pwm"][fan][0], interfaces[ifaceTypeFromMethod(method)],
            property);
...
void set(uint64_t target, std::vector<std::string>& fanList)
{
    ...
    for (auto& fan : fanList)
    {
        try
        {
            auto paths(pathMap["pwm"].find(fan));

            if (pathMap["pwm"].end() == paths)
            {
                // try again, maybe it was a sensor name instead of a fan name
                for (const auto& [fanName, sensors] : pathMap["pwm"])
                {
                    for (const auto& path : sensors)
                    {
                        ...
                        if (sensor.size() > 0)
                        {
                            ...
                            if (getNumber(sensor) == getNumber(fan))
                            {
                                paths = pathMap["pwm"].find(fanName);
            ...
            if (pathMap["pwm"].end() == paths)
            {
                std::cout << "Could not find pwm path for fan: " << fan
                          << std::endl;
                continue;
            }
...
{
    log<level::INFO>("Fanctl running init CLI");
    ...
    log<level::INFO>("Fanctl instantiating status method");
    // status method
...
- list of target sensors to set)";
    ...
    log<level::INFO>("Fanctl finished init CLI");
